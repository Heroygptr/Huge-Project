...
def k_line(symble, title, line_1=5, line_2=10, last = -30, adjust = "qfq", dprint = False):
  '''
  过去n天个股历史行情均线数据（k线）
  ...
  # 均线数据
  df3[str(line_1)] = df3.close.rolling(line_1).mean()
  df3[str(line_2)] = df3.close.rolling(line_2).mean()
  df3.tail()
  ...
  # 画均线
  plt.plot(df3[str(line_1)].values, alpha = 0.5, label='MA'+str(line_1))
  plt.plot(df3[str(line_2)].values, alpha = 0.5, label='MA'+str(line_2))
...
k_line(symble = "sh601727",  line_1=5, line_2=10,last = -30, adjust = "qfq", dprint = False, title = "上海电气")